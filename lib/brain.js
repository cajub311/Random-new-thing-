...
/**
 * Same as runAgent but uses callOpenAIStream for each model turn so the client
 * can show token-by-token output. callOpenAIStream must return a merged message
 * object like callOpenAI (with optional tool_calls).
 */
export async function runAgentStream({
  provider, callOpenAIStream, messages, model, apiKey,
  skills, toolDefs, maxSteps = 8, ctx, onStep, onStreamDelta, onToolStart,
  temperature, max_tokens,
} = {}) {
  const genOpts = { temperature, max_tokens };
  const trace = [];
  const convo = [...messages];

  for (let step = 0; step < maxSteps; step++) {
    const msg = await callOpenAIStream(provider, convo, model, apiKey, {
      tools: toolDefs,
      ...genOpts,
      onDelta: d => onStreamDelta?.(d),
    });
    ...
    for (const tc of toolCalls) {
      ...
      await onToolStart?.({ step: step + 1, tool: fname, args });
      let result;
      if (!skill) {
  ...
  const finalMsg = await callOpenAIStream(provider, convo, model, apiKey, {
    tools: [],
    ...genOpts,
    onDelta: d => onStreamDelta?.(d),
  });
...
export async function runAgent({
  ...
  for (let step = 0; step < maxSteps; step++) {
    const msg = await callOpenAI(provider, convo, model, apiKey, { tools: toolDefs, ...genOpts });
    const toolCalls = msg.tool_calls || [];
    const assistantEntry = { role: 'assistant', content: msg.content || '' };
    if (toolCalls.length) assistantEntry.tool_calls = toolCalls;
    convo.push(assistantEntry);
  ...
  const finalMsg = await callOpenAI(provider, convo, model, apiKey, { tools: [], ...genOpts });
  return {
    reply: finalMsg.content || 'Reached the max tool-call step limit.',
    steps: maxSteps,
    trace,
