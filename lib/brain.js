...
// Scan an agent trace and return any URLs that were actually visited via
// fetch_url or surfaced via web_search. Used to append a "Sources" footer
// if the model forgot to cite.
export function extractCitations(trace) {
  const urls = new Map();  // url -> title
  for (const step of trace || []) {
    if (step.tool === 'fetch_url' && step.args?.url) {
      urls.set(step.args.url, step.args.url);
    }
    if (step.tool === 'web_search' && Array.isArray(step.result?.results)) {
      for (const r of step.result.results) {
        if (r?.url) urls.set(r.url, r.title || r.url);
      }
    }
  }
...
export function appendCitationsIfMissing(reply, trace) {
  const cites = extractCitations(trace);
  if (!cites.length) return reply;
  const replyLc = String(reply || '').toLowerCase();
  // Only cite URLs the model actually used in its answer — fall back to top 5
  // if it cited none at all.
  const mentioned = cites.filter(c => replyLc.includes(c.url.toLowerCase()));
  const alreadyLinks = /\]\(https?:\/\//.test(reply);
  if (alreadyLinks && mentioned.length) return reply;
  const toList = mentioned.length ? mentioned : cites.slice(0, 5);
  const footer = '\n\n---\n**Sources**\n' + toList.map(c => `- [${c.title}](${c.url})`).join('\n');
  return reply + footer;
}
