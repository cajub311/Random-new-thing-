 ...
 *  - maxSteps, ctx, onStep
 *  - onBeforeLLM({ step, wrapUp? }) — before each model call (loop or max-step wrap-up).
 *  - onBeforeTool({ step, tool, args, rawArguments }) — before each tool (incl.
 *    final_answer); args may include __parse_error.
 */
export async function runAgent({
  provider, callOpenAI, messages, model, apiKey,
  skills, toolDefs, maxSteps = 8, ctx, onStep, onBeforeLLM, onBeforeTool,
}) {
  const trace = [];
  const convo = [...messages];

  for (let step = 0; step < maxSteps; step++) {
    await Promise.resolve(onBeforeLLM?.({ step: step + 1 }));
    ...
    for (const tc of toolCalls) {
      ...
      await Promise.resolve(onBeforeTool?.({
        step: step + 1,
        tool: fname,
        args,
        rawArguments: typeof fargs === 'string' ? fargs : String(fargs),
      }));
  ...
  await Promise.resolve(onBeforeLLM?.({ step: maxSteps + 1, wrapUp: true }));
