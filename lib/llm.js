...
export async function callAnthropic(provider, messages, model, apiKey) {
  const m = model || provider.defaultModel;
  const { system, messages: anthroMsgs } = openAIMessagesToAnthropic(messages);
  const body = {
    model: m,
    max_tokens: 8192,
    messages: anthroMsgs,
...
export async function chatCompletion(provider, messages, model, apiKey, opts = {}) {
  ...
  if (provider.format === 'anthropic')   return callAnthropic(provider, messages, model, apiKey);
