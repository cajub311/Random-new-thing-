...
import { consumeOpenAIChatStream } from './openaiStream.js';

export function clampGenerationOpts({ temperature, max_tokens } = {}) {
  const t = temperature === undefined || temperature === null ? 0.7 : Number(temperature);
  const maxTok = max_tokens === undefined || max_tokens === null ? 4096 : Number(max_tokens);
  return {
    temperature: Number.isFinite(t) ? Math.min(2, Math.max(0, t)) : 0.7,
    max_tokens: Number.isFinite(maxTok) ? Math.min(128000, Math.max(1, Math.round(maxTok))) : 4096,
  };
}

export async function callOpenAI(provider, messages, model, apiKey, { tools, temperature, max_tokens } = {}) {
...
/**
 * One streamed chat completion; merges SSE into a single message (like callOpenAI).
 * Only for OpenAI-compatible providers (format === 'openai').
 */
export async function streamOpenAIMessage(provider, messages, model, apiKey, opts = {}) {
  if (provider.format !== 'openai') {
    return callOpenAI(provider, messages, model, apiKey, opts);
  }
  const gen = clampGenerationOpts(opts);
  const headers = {};
  if (apiKey) headers['Authorization'] = `Bearer ${apiKey}`;
...
export async function callCohere(provider, messages, model, apiKey, opts = {}) {
  const gen = clampGenerationOpts(opts);
...
export async function callHuggingFace(provider, messages, model, apiKey, opts = {}) {
  const gen = clampGenerationOpts(opts);
  const m = model || provider.defaultModel;
  const url = buildUrl(provider, m);
  ...
  const maxNew = Math.min(gen.max_tokens, 4096);
  const res = await fetch(url, {
    method: 'POST',
    headers: { 'Authorization': `Bearer ${apiKey}`, 'Content-Type': 'application/json' },
    body: JSON.stringify({
...
export async function chatCompletion(provider, messages, model, apiKey, opts = {}) {
  ...
  if (provider.format === 'cohere')      return callCohere(provider, messages, model, apiKey, opts);
  if (provider.format === 'huggingface') return callHuggingFace(provider, messages, model, apiKey, opts);
  throw new Error(`Unknown format: ${provider.format}`);
}

// Probe a local provider. Returns { reachable, models[] }.
export async function probeLocal(provider) {
  if (!provider.local) return { reachable: false, models: [] };
  try {
    if (provider.name.startsWith('Ollama')) {
      const res = await fetch(`${provider.baseUrl}/api/tags`, { signal: AbortSignal.timeout(800) });
      if (!res.ok) return { reachable: false, models: [] };
      const data = await res.json();
      const models = (data.models || []).map(m => m.name);
      return { reachable: true, models };
    }
    // LM Studio / llama.cpp — standard OpenAI /v1/models endpoint.
    const res = await fetch(`${provider.baseUrl}/v1/models`, { signal: AbortSignal.timeout(800) });
    if (!res.ok) return { reachable: false, models: [] };
    const data = await res.json();
